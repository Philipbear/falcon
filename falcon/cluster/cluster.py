...
def hierarchical_clustering(
    ...
    dist_matrix = pairwise_dist_matrix.toarray()
    np.fill_diagonal(dist_matrix, 0)
    # Run scipy clustering.
    if dist_matrix.shape[0] > 1:
        dist_matrix = squareform(dist_matrix)
        linkage = hierarchy.linkage(dist_matrix, method=config.linkage)
        clusters = hierarchy.fcluster(linkage, eps, criterion="distance")
